...
public class MainFrame extends JFrame {
    ...
    private String serverIP=Config.getValue("serverIP");
     ...
     * 嵌套子菜单
     */
    private JMenuItem miGather, miMatchLog, miMatchTrans, miSave, miSend, miShow, miExit, miCheck, miAbout;

    /**
     * 数据采集和数据显示的日志和物流的选项卡的容器
     */
    private JTabbedPane tpGather, showPane;
    private JPanel p, pLog, pTran, pLogId, pName, pLocation, pIP, pLogStatus, pLogButton, pTransId, pAdress, pHandler,
            pReceiver, pTranStatus, pTranButton;
    private JLabel lblLogId, lblName, lblLocation, lblIP, lblLogStatus, lblTransId, lblAdress, lblHandler, lblReceiver,
            lblTranStatus;

    /**
     * 文本框
     */
    private JTextField txtLogId, txtName, txtLocation, txtIP, txtTransId, txtAdress, txtHandler, txtReceiver;
     ...
     * 按钮
     */
    private JButton btnLogConfirm, btnLogReset, btnTranConfirm, btnTranReset, btnGather, btnMatchLog, btnMatchTrans,
            btnSave, btnSend, btnShow;
    ...
    /**
     * 卡片式布局（ 数据采集和数据显示），把数据采集界面和数据显示的界面分别做成了两个卡片
     */
     ...
     * 声明匹配日志列表
     */
    private List<MatchedLogRec> matchedLogs;

    /**
     * 声明匹配物流列表
     */
    private List<MatchedTransport> matchedTrans;
    ...
    public MainFrame() {
        ...
        // 设置全局字体
        InitGlobalFont(new Font("楷体", Font.PLAIN, 17));
    ...
    private void initMenu() {
        ...
        miCheck.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                // 显示消息对话框
                JOptionPane.showMessageDialog(null, "本系统实现日志信息及物流信息的采集、过滤分析匹配、保存、发送及显示功能", "帮助",
    ...
    private void initLogGatherGUI() {
        ...
        lblLocation = new JLabel("地址：");
        lblLocation.setPreferredSize(new Dimension(65, 20));
    ...
    private class GatherLogListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            // 获取日志ID
            try {
                id = Integer.parseInt(txtLogId.getText().trim());
            } catch (Exception i) {
                JOptionPane.showMessageDialog(null, "ID仅能为数字!", "错误提示", JOptionPane.ERROR_MESSAGE);
                i.printStackTrace();
                return;
            }
            ...
            if (ip.matches(validIpRegex) != true) {
                JOptionPane.showMessageDialog(null, "请重新输入主机IP(*.*.*.*)!", "错误提示", JOptionPane.ERROR_MESSAGE);
                return;
            }
    ...
    private class GatherTransListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            int id = -1;
            try {
                id = Integer.parseInt(txtTransId.getText().trim());
            } catch (Exception i) {
                JOptionPane.showMessageDialog(null, "ID仅能为数字!", "错误提示", JOptionPane.ERROR_MESSAGE);
                i.printStackTrace();
                return;
    ...
    private class SaveDataListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            if (matchedLogs != null && matchedLogs.size() > 0) {
                ...
                logRecService.saveMatchedLogRec(matchedLogs);
        ...
        public void actionPerformed(ActionEvent e) {
            try {
                ...
                if (matchedLogs != null && matchedLogs.size() > 0) {
                    ...
                    logSocket = new Socket(serverIP, 6666);
                    // 创建用于序列化匹配日志信息对象的输出流
                ...
                if (matchedTrans != null && matchedTrans.size() > 0) {
                    // 创建Socket发送物流信息，日志信息发送到服务器的6668端口
                    transSocket = new Socket(serverIP, 6668);
                    // 创建用于序列化匹配物流信息对象的输出流
                    ...
                    matchedTrans.clear();
                    // 弹出发送成功提示框
                    JOptionPane.showMessageDialog(null, "匹配的物流数据已发送到服务器！", "提示", JOptionPane.PLAIN_MESSAGE);
                } else {
                    JOptionPane.showMessageDialog(null, "没有匹配的物流数据！", "提示", JOptionPane.WARNING_MESSAGE);
                }
